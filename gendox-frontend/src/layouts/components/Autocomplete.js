...
import { fetchAutocompleteSearchData } from "src/store/apps/autocompleteSearch/autocompleteSearch";
...
const AutocompleteComponent = ({ hidden, settings, user }) => {
    ...
    if (searchValue.length > 0) {
      const orgOptions = user.organizations
        ...
        .map((org) => ({
          ...
          link: `/gendox/home/?organizations=${org.id}&projectId=${org.projects?.[0]?.id ?? null}`,
        }));

        const projectOptions = user.organizations.flatMap((org) =>
          org.projects
            .filter((proj) =>
            ...
            .map((proj) => ({
              ...
              icon: "mdi:view-grid-outline",
              category: "Projects",
              link: `/gendox/home/?organizations=${org.id}&projectId=${proj.id}`,
            }))
        );
  
      setOptions([
        ...defaultSuggestionsData.flatMap((category) => category.suggestions),
        ...orgOptions,
        ...projectOptions,
      ]);
    } else {
  ...
  }, [searchValue, user.organizations]);
          ...
          >
            <Box sx={{ top: 0, width: "100%", position: "sticky" }}>
              <Autocomplete
                ...
                groupBy={(option) =>
                  searchValue.length ? categoryTitle[option.category] || option.category : ""
                }
                sx={{
                  "& + .MuiAutocomplete-popper": {
                    ...(searchValue.length
