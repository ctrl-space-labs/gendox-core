...
import CreateApiKeyDialog from 'src/views/pages/organization-settings/advanced-components/api-keys/CreateApiKeyDialog'
import NameChangeDialog from 'src/views/pages/organization-settings/advanced-components/api-keys/NameChangeDialog'
...
const ApiKeysAdvancedOrganizationSettings = () => {
  ...
  const handleUpdateApiKey = async newName => {
    try {
      ...
      await apiKeyService.updateApiKey(organizationId, selectedApiKeyId, payload, token)
      dispatch(
        fetchApiKeys({
          organizationId,
          token
  ...
  const handleDeleteProviderKey = async () => {
    try {
      await apiKeyService.deleteApiKey(organizationId, selectedApiKeyId, token)
      dispatch(
        fetchApiKeys({
          organizationId,
          token
            ...
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Box sx={{ display: 'flex', ml: 1 }}>
                ...
                <Tooltip title="Change Api Key's name">
                  <IconButton onClick={() => handleEditToggle(item?.id, item?.name)} color='primary'>
