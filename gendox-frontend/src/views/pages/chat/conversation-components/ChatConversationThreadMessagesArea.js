...
const ThreadMessagesArea = ({
  ...
  function buildDisplayItems(rawMessages = []) {
    ...
    rawMessages.forEach(msg => {
      ...
      // ────────────────────────────────────────────────────────────────
      const hasToolCalls = msg.role === 'assistant' && Array.isArray(msg.toolCalls) && msg.toolCalls.length > 0
  ...
  const displayItems = useMemo(() => buildDisplayItems(currentThread?.messages), [currentThread?.messages])
        ...
        <Box sx={{ flex: 1, p: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {auth?.user?.id && displayItems.length > 0 ? (
            displayItems.map((item, index) => {
              ...
              const uiAttachments = Array.isArray(message.attachments)
                ? message.attachments.map(doc => toMessageAttachmentUI(doc))
                : []
              return (
                <React.Fragment key={index}>
                  ...
                  >
                    {/* stack container so attachments appear ABOVE bubble */}
                    <Box
                      sx={{
                        maxWidth: '90%',
                        display: 'flex',
