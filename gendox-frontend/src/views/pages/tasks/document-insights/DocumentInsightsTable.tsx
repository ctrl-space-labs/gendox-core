...
interface DocumentInsightsTableProps {
  selectedTask: any
}

const DocumentInsightsTable = ({ selectedTask }: DocumentInsightsTableProps) => {
  ...
  const openDialog = (dialogType: string, node: any = null, forceEditMode: boolean = false) => {
  ...
  const isGenerating = Object.values(isInsightsGeneratingCells || {}).some((v: any) => v === true)

  return (
    <>
      <div className="p-6 overflow-x-auto bg-accent/30 rounded-lg mb-6 border border-border">
        <HeaderSection
          ...
          documents={documents}
          questions={questions}
          hasGeneratedContent={(docId: string, questionId?: string) => {
            if (questionId) {
              return answers.some((answer: any) => answer.documentNodeId === docId && answer.questionNodeId === questionId)
            }
            return answers.some((answer: any) => answer.documentNodeId === docId)
          }}
        />
