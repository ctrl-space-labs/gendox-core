...
Implementation plan, revised 2026-09-17 while building it, with implementation notes folded in as tickets land. **Status: in progress** — 526.1 (provider seam and Firecrawl adapter), 526.2 (schema, entities, repository), 526.4 (entitlement and budget), 526.5 (the integration run) and 526.6 (REST surface) are implemented, and 526.4–526.6 were verified end to end against a live site on 2026-09-22. 526.3 (`allow_embed` and the unified sources list) is deferred into 526.7, so that it is decided together with the frontend it belongs to. The ticket itself carries only Goal / Scope / Notes / Relationships — this document is the plan, and the issue body was deliberately left free of it.

The plan as first written is kept unchanged in [`issue-526-web-scraping-integration-2026-09-10.md`](issue-526-web-scraping-integration-2026-09-10.md). Everything below reflects what the Firecrawl evaluation and the first two tickets actually settled; where the two files disagree, this is the one that holds.
...
- `WebScrapeProviderUtils` — constructor-injected `List<WebScrapeProvider>`, resolve by `supports(...)`, throw `GendoxException("WEB_SCRAPE_PROVIDER_NOT_SUPPORTED", …, BAD_REQUEST)`. Direct analogue of `AiModelUtils`, and it lives beside it in `gendoxcoreapi/utils/`, where every `*Utils` class in the project lives. It also resolves the API key (organization connector first, then the platform key).
- `FirecrawlWebScrapeProvider` — `supports("FIRECRAWL")`.
- Provider-neutral DTOs: `WebScrapeTargetDTO{seedUrl, apiKey, crawlLimit, includePaths, excludePaths}`, `WebCrawlResultDTO{List<DiscoveredPageDTO>}`, `DiscoveredPageDTO{url, title}`, `WebPageContentDTO{url, title, markdown, contentHash, fetchedAt}`. A provider returning HTML converts inside its own adapter; the contract stays markdown. These DTOs live in `model/dtos/integrations/webScrape/`, following the existing `model/dtos` subpackages and `DTO` suffix; provider-specific request/response DTOs live under `model/dtos/integrations/webScrape/firecrawl/request` and `/response`, mirroring `ai/engine/model/dtos/<provider>/request|response`. The crawl result also carries `limitReached`, so a deep crawl that stopped at its page limit is never read as evidence of removal (Phase 3).

Firecrawl calls go through a `RestClient` the adapter creates from the shared `RestTemplate` bean (`RestClient.create(restTemplate)`), so no configuration class changes and the project's timeouts and tracing apply (house style is `RestClient`, not `RestTemplate`):
...
**Credentials.** Platform key from `gendox.integrations.web-scrape.firecrawl.api-key: ${FIRECRAWL_KEY:}`, following the existing `OPENAI_KEY` / `COHERE_KEY` pattern, and wired through the docker compose files like the other provider keys. An org may override with its own key via `organization_connectors` (`config.apiKey`) — add `WEB_SCRAPE_FIRECRAWL` to `ConnectorTypesConstants`. Resolution order: org connector, else platform key.

**Implemented 2026-09-16.** Error codes per call: `WEB_SCRAPE_MAP_FAILED`, `WEB_SCRAPE_SCRAPE_FAILED`, `WEB_SCRAPE_CRAWL_FAILED`, plus `WEB_SCRAPE_PAGE_FAILED` for a page that came back with `statusCode >= 400` or empty markdown, and `WEB_SCRAPE_API_KEY_NOT_FOUND` when neither key exists. Non-page links (`.xml`, `.json`, images, …) are dropped by suffix. Deliberately left for later, to keep the first version small: retries for `429`/`5xx`, `includePaths`/`excludePaths` filtering (the DTO fields exist but are ignored), block-page detection in the markdown, and per-status error codes.
...
**`integrations` — two generic columns and one settings payload.** Deliberately *not* following the `directory_path` / `repository_head` precedent: those columns each serve a single integration type and have turned the shared table into a bag. Instead: `run_interval_minutes int` (NULL means not scheduled, so every existing integration keeps today's behaviour), `last_run_at timestamp`, and `config jsonb not null default '{}'::jsonb` holding whatever a type needs — for web scraping `{ provider, crawlPageLimit }`, later `includePaths` / `excludePaths` without a migration. Nothing in the schema names this feature, so the next integration type adds its settings without touching the table. The two columns stay columns because they are cross-type concepts rather than per-type settings, and because `last_run_at` is state the system writes on every run: mixing it into the payload would let a finishing run and a user edit overwrite each other. `config` mirrors `organization_connectors.config`, already mapped in Java with `@JdbcTypeCode(SqlTypes.JSON)`. `integration.url` holds the seed URL for this type.

**`subscription_plans` — one new column** `web_scrape_pages_monthly_limit int NOT NULL DEFAULT 0`, then re-seeded per `sku` the way `V20260911_120000__Reconcile_subscription_plans_with_pricing.sql` seeds `project_limit` and `document_pages_limit`: `gd-free-001` and `gd-basic-001` stay `0`, `gd-pro-001` and `gd-business-001` get a positive number. One column does both jobs: `> 0` **is** the PRO entitlement, and the value **is** the monthly page budget. No separate feature flag. Follow the two conventions that migration states: **limits are per seat** (multiplied by `organization_plan.number_of_seats` at check time, via `SubscriptionValidationService.effectiveLimit`, where `null` means zero) and **9999 means unlimited** under the fair-usage policy.
...
**Monthly page usage — no counter column.** Scraped pages are counted straight from `web_scrape_pages.last_scraped_at` within the billing period, the way the merged subscription code counts websites, projects and document pages from their own tables. `organization_daily_usage` is left alone: its trigger is commented out and its counters are filled by the native `aggregateDailyUsage` query draining `audit_log_queue`, code that has already caused deadlocks and that a new counter would have to touch. Counting from the table also cannot drift, since it does not depend on an aggregation run having caught up. The supporting index `(integration_id, last_scraped_at)` ships with the table.
...
1. **Due check** — return empty unless `now >= last_run_at + run_interval_minutes`, skipping entirely when `run_interval_minutes` is NULL. This is where per-integration frequency lives; the shared poller visits every active integration every 30 minutes regardless.
...
5. **Implemented differently (526.5).** The service uploads each page itself through `documentService.uploadSingleFile(...)` — which stores the file, upserts the document and queues it for splitting and embedding in one call — and returns an **empty** map. `IntegratedFileDTO` carries no field pointing back at the page it came from, and the shared handler discards the `DocumentInstance` it gets back, so there was no way to stamp `document_instance_id` through the existing path. An empty map is already read by the caller as *no update*.
...
- [`IntegrationConfiguration.integrationHandler`](gendox-core-api/src/main/java/dev/ctrlspace/gendox/gendoxcoreapi/configuration/IntegrationConfiguration.java) — **not touched.** Stamping `document_instance_id`, `content_hash`, `last_scraped_at` and `status = SCRAPED` happens in the update service, right after its own upload call, which is what made the shared handler unnecessary. Nothing is written to a usage counter: `last_scraped_at` is the record, and the monthly count is derived from it.

Deselecting a page deletes its document via `documentService.deleteAllDocumentInstances(...)` and clears `document_instance_id` **and `content_hash`** — otherwise unselecting leaves stale content in the agent's context, and re-selecting the page would match the old hash, skip the upload, and leave it `SCRAPED` with no document.

**Split for the buttons (526.6).** `checkForUpdates` keeps the due check and the entitlement check, then calls two public methods, `discoverPages` and `scrapeSelectedPages`. A third, `deepDiscoverPages`, runs the link-following crawl. The endpoints call them separately; the scheduled run is their combination. No logic moved, only code.
...
- **Every path is nested under `/organizations/{organizationId}/`, and each handler loads the integration through a guard that answers `404` unless it belongs to that organization.** The authority extractors resolve an organization or a project from the path; there is none for an integration id. So the organization in the path is the only thing `@PreAuthorize` can check, and the integration has to be tied back to it explicitly. The guard answers `404` rather than `403`, which would confirm that the id exists somewhere else.
- **Select-all is server-side** — a single `UPDATE … WHERE integration_id = ? AND status <> REMOVED`. A site can hold thousands of pages; posting every id would be wrong. The selection flag is nullable on the wire and a body that omits it is rejected: a primitive would default to *unselected*, which on the next run deletes documents nobody asked to remove.
- All three `POST` actions are `@Async` + `@SchedulerLock(name = "webScrapeManualTrigger-#{#integrationId}", …)`, mirroring `IntegrationService.triggerForOrganization`. **Repeated clicks must not fan out into repeated bills**, so the lock has to be effective in the deployed configuration and not merely present on the method — worth confirming per environment rather than assuming. They bypass the due check but **not** entitlement or budget. A deep crawl bills per page visited, so its `crawl_page_limit` is checked against the remaining monthly budget before it starts, and a deep crawl with no configured limit is refused: with no limit there is no bound to check.
- **The checks run in the request thread, the work does not.** An `@Async` method throws after the `202` has already gone out, where nothing can surface it, so each action validates entitlement and budget first and only then dispatches.
...
**Done 2026-09-22, against the dev database and a live site.** 526.4–526.6 were verified through the API instead of unit tests: Map discovery returned a subset of the site, a deep crawl of the same site stored roughly four times as many pages, a selected page produced a document with sections in the target project, a second scrape of the unchanged page reused the same `document_instance_id`, deselecting it deleted the document, and a later discovery preserved both the selection and the document while refreshing titles. The negative paths each answered as intended: an integration id from another organization, a selection body without the flag, an interval below a day from a non-admin, and a deep crawl with no configured page limit. The unit tests listed below remain unwritten.

- `cd gendox-core-api && mvn test` — unit tests below.
- `mvn flyway:migrate` against the target environment (connection details come from that environment, not from here), then confirm `web_scrape_pages` exists, the four `integrations` columns are present, the plan seeding gives Free/Basic `0` and Pro/Business `> 0`, and **every pre-existing `organization_web_sites` row has `allow_embed = true`** (a row left `false` would silently drop an embed domain once #527 enforces the flag).
- New tests:
  - `FirecrawlWebScrapeProviderTest` — map, scrape and deep-crawl parsing plus their failure paths, against recorded fixtures in `src/test/resources/firecrawl/`, never the live API. `WebScrapeProviderUtilsTest` — provider lookup and the platform-key fallback. Both bind `MockRestServiceServer` to the shared `RestTemplate`, which also covers the `RestClient` the adapter wraps around it.
...
- Frequency: set `run_interval_minutes = 1440`, run the poller twice inside 30 minutes, confirm the second pass is a no-op. Confirm a non-admin is rejected setting an interval below 1440 and a super admin is not.
...
- **Decided 2026-09-22: a deep crawl's page list is read straight from the crawl results.** It saves a request and does not depend on the index behaviour above. Confirmed in practice — on a site Map reported as nine pages, the deep crawl stored thirty more.
- **Response parsing rules observed on 2026-09-14:**
  ...
  - Store `sourceURL` (the requested URL, matching Map output), not `url` (post-redirect, may gain a trailing slash), so `(integration_id, url)` stays stable. **Not enough on its own (observed 2026-09-22):** Map and deep crawl report the same page without and with a trailing slash respectively, so it became two rows under the unique key — billable twice, and two documents of identical content each answering the agent separately. Discovery normalizes the URL before the upsert.
...
- **Subscription limits were reworked on `dev` (merged 2026-09-16).** `SubscriptionValidationService` now resolves every limit through `effectiveLimit(planLimit, seats)`; `9999` means unlimited; stock-like limits (websites, projects, document pages) are counted straight from their own tables, while period limits (messages, uploads, storage) still come from `organization_daily_usage`; and monthly windows come from `BillingWindowUtils.currentBillingPeriod`, anchored on the subscription start date. Phases 2 and 5 follow these conventions.
