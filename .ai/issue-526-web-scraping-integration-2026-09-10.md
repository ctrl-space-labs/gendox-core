...
- `WebScrapeProvider` — `crawl(WebScrapeTarget)` returns URLs + titles only (no bodies) from the provider's cheap discovery; `deepCrawl(WebScrapeTarget)` returns the same `WebCrawlResult` from a link-following crawl, run only on user request; `scrape(WebScrapeTarget, url)` returns one page as markdown; plus `getSupportedProviderNames()` / `supports(String)`.
...
- **Response parsing rules observed on 2026-09-14:**
  - Map `links[]` entries can lack `title` and can be non-page URLs (e.g. `sitemap.xml`): `DiscoveredPage.title` is nullable and non-page links are filtered out.
