...
## Issue Tracker Rules (MANDATORY)

**NEVER create any ticket/issue (GitHub, or any other tracker) unless the user explicitly asks you to create it.**

- Planning, drafting, brainstorming, or writing out proposed tickets is fine — but stop at the draft. Do not call `gh issue create` or any equivalent without an explicit "create the tickets" instruction from the user.
- When the user *does* explicitly ask, every created ticket MUST carry the label `needs-review`.
- This applies to issues, sub-issues, epics, and any bulk/scripted creation.
...
- No absolute local paths, usernames, home directories, or session/scratchpad paths.
- No references to private trackers or private-repo issues (e.g. `gendox-development-management`); keep public plans pointing at public issues only.
- **No details of unfixed security weaknesses.** Describing an exploitable gap and its call site publishes it before the fix ships. State the design requirement that follows from it and note that the specifics belong in the private tracker.
- Internal pricing or plan limits stay abstract (`Pro n`), not actual figures.
