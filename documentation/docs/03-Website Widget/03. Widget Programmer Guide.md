...
| `window.gendox.widget.addLocalContextRequestCallback(callback)` | Register a callback that runs when the chat asks for local context. The callback receives `(event, sendResponse)` and may either return one `{ contextType, value }` object, return an array of them, or call `sendResponse(...)` directly for async work. See example in §5.4. |
| `window.gendox.widget.removeLocalContextRequestCallback(callback)` | Remove a callback previously added with `addLocalContextRequestCallback`. |
...
Tool calls are triggered by the agent; the widget sends `gendox.events.chat.message.tool_calls.request` (see Events section). For full tool-use documentation and payload format, see [Agent Tool Use and Website Tool Support](./02.%20Agent%20Tool%20Use%20and%20Website%20Tool%20Support.md).
...
Use `addLocalContextRequestCallback` to register one or more callbacks that run when the chat asks for local context. By default, the SDK auto-registers a selected-text callback. Your callbacks can add more context, or you can disable the default selected-text callback with `data-gendox-local-context-selected-text-enabled="false"`.

Return a single object:

```javascript
window.gendox.widget.addLocalContextRequestCallback(function () {
  return {
...
window.gendox.widget.addLocalContextRequestCallback(callback);
window.gendox.widget.removeLocalContextRequestCallback(callback);
```
