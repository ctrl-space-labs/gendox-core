-- Create the API_INTEGRATION type if it doesn't exist
...
ALTER TABLE gendox_core.integations
...
(
    ...
    FOREIGN KEY (content_id) REFERENCES gendox_core.document_instance (content_id)
);
comment on table gendox_core.api_keys is 'Table to store temporary integration file checks';

-- Create the column if it doesn't exist and ensure it's unique
