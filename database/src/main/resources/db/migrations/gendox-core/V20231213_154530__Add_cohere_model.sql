...
where not exists(SELECT * FROM gendox_core.ai_models where model = 'command');
...
where not exists(SELECT * FROM gendox_core.ai_models where model = 'embed-multilingual-v3.0');
